...
import type { MatchParticipant, MatchStatus, MatchView } from '../lib/ladder-types';
...
export async function loadParticipants(matchIds: string[]): Promise<ParticipantRow[]> {
  ...
  return sql()<ParticipantRow[]>`
    select mp.match_id, mp.player_id, mp.team, mp.rating_before, mp.rating_after, mp.rating_delta,
           mp.outcome, p.steam_id, coalesce(p.display_name, p.persona_name) as persona_name, p.avatar_url
...
export function toView(m: MatchRow, participants: ParticipantRow[]): MatchView {
  return {
    id: m.id,
    mode: m.mode,
