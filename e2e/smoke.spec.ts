...
test('modding docs honor the versioned routing and metadata contract', async ({ page, request }) => {
  ...
  const versionOptions = await page
    .getByLabel('Documentation version')
    .locator('option')
    .evaluateAll((options) => options.map((option) => (option as HTMLOptionElement).value));
  if (versionOptions.length > 1) {
    const targetId = versionOptions.find((value) => value !== snapshotId);
    if (targetId) {
      await page.getByLabel('Documentation version').selectOption(targetId);
      await expect(page).toHaveURL(new RegExp(`/modding/${targetId}/`));
    }
  }
